 ...
 */
?>
<div id="edit_payment_modal_{{$payment->id}}" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel"
     aria-hidden="true">
        ...
        <div class="modal-content">
            <form action="{{route('update-payment',[$payment->id])}}" method="post">
                ...
                <div class="modal-header modal-colored-header bg-success">
                    <h4 class="modal-title" id="standard-modalLabel">Update Payment Details</h4>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-hidden="true"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-1">
                        <label for="plan" class="form-label">Membership Plan</label>
                        <select name="plan_id" id="membership_id" class="form-control @error('plan_id') is-invalid @enderror"
                            ...
                            @foreach($plans as $plan)
                                <option value="{{$plan}}" data-amount="{{$plan->amount}}"
                                        data-validity="{{$plan->validity_period}}" @if($plan->id==$payment->plan_id) selected @endif>{{$plan->plan_name}}</option>
                            @endforeach
                        </select>
                        @error('plan_id')
                    ...
                    <div class="mb-1">
                        ...
                        <input readonly type="text" name="validity_period" id="validity_id"
                    ...
                    @else
                        <div class="mb-1">
                            ...
                            <div class="input-group">
                                <input type="text" class="form-control" placeholder="Search..."
                                       id="client_search" value="{{$payment->user->name}}">
                        ...
                        <div class="mb-1 col">
                            <label for="user_id" class="form-label">Client</label>
                            <select name="user_id" class="form-control @error('user_id') is-invalid @enderror"
                                    value="{{ old('user_id') }}" autocomplete="user_id" autofocus id="client_edit_id"
                                    required>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-success"><i class="mdi mdi-content-save"></i> Update Payment
                    </button>
                </div>
            </form>
        </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
                    ...
                    success: function (response) {
                        ...
                        if (len > 0) {
                            for (var i = 0; i < len; i++) {
                                ...
                                $("#client_edit_id").append(option);
                            }
                        } else {
                            $('#notify_modal').modal('show');
                            $('#client_search').val(null);
                            $("#client_edit_id").find('option').remove()
                        }
                    }
                });
            });

            $('#client_search_btn').find(function () {
                var text = $('#client_search').val();
                $.ajax({
                    url: 'search-client/' + text,
                    type: 'get',
            ...
            $('#membership_id').change(function () {
                ...
                $("#validity_id").val(validity + " days");
                $("#amount_id").val("$ " + amount);
            });

            $('#membership_id').find(function () {
                var amount = $(this).find(':selected').data('amount');
                var validity = $(this).find(':selected').data('validity');
                $("#validity_id").val(validity + " days");
                $("#amount_id").val("$ " + amount);
